...
class Sim_GUI(QWidget):
    ...
    def initUI(self):
        ...
        self.weapons = {"150":"ADProject/img/150.png", "160":"ADProject/img/160.png", "200":"ADProject/200.png"}
        self.weaponImage = QPixmap()
        self.weaponImage.load(self.weapons["160"])
        self.weaponImage = self.weaponImage.scaled(150, 150)
        self.displayImage = QLabel()
        self.displayImage.setPixmap(self.weaponImage)
        self.nowWeaponText = "160"
        ...
        averageDistroyLabel = QLabel("평균 파괴 회수 :", self)
        self.averageDistroy = QLineEdit()
        self.averageDistroy.setAlignment(Qt.AlignRight)
        self.averageDistroy.setReadOnly(True)

        maxDistroyLabel = QLabel("최소 파괴 회수 :", self)
        self.maxDistroy = QLineEdit()
        self.maxDistroy.setAlignment(Qt.AlignRight)
        self.maxDistroy.setReadOnly(True)

        minDistroyLabel = QLabel("최대 파괴 회수 :", self)
        self.minDistroy = QLineEdit()
        self.minDistroy.setAlignment(Qt.AlignRight)
        self.minDistroy.setReadOnly(True)

        # 오른쪽 레이아웃에 위젯들을 추가
        rightLayout = QGridLayout()

        rightLayout.addWidget(emptyPlace, 0, 0, 1, 1)
        rightLayout.addWidget(startStarsLabel, 0, 1, 1, 1, Qt.AlignRight)
        rightLayout.addWidget(self.startStars, 0, 2, 1, 1)
        rightLayout.addWidget(emptyPlace, 0, 3, 1, 1)
        rightLayout.addWidget(toStarsLabel, 0, 3, 1, 1, Qt.AlignRight)
        rightLayout.addWidget(self.toStars, 0, 4, 1, 1)
        rightLayout.addWidget(emptyPlace, 0, 5, 1, 1)
        rightLayout.addWidget(tryNumsLabel, 0, 6, 1, 1, Qt.AlignRight)
        rightLayout.addWidget(self.tryNums, 0, 7, 1, 1)
        rightLayout.addWidget(emptyPlace, 0, 8, 1, 1)

        rightLayout.addWidget(startSimulationButton, 0, 9, 1, 2)

        rightLayout.addWidget(self.simulationStatus, 1, 0, 1, 11)

        rightLayout.addWidget(averageUsedMesoLabel, 2, 7, 1, 1, Qt.AlignRight)
        rightLayout.addWidget(self.averageUsedMeso, 2, 8, 1, 1)

        rightLayout.addWidget(minUsedMesoLabel, 3, 7, 1, 1, Qt.AlignRight)
        rightLayout.addWidget(self.minUsedMeso, 3, 8, 1, 1)

        rightLayout.addWidget(maxUsedMesoLabel, 4, 7, 1, 1, Qt.AlignRight)
        rightLayout.addWidget(self.maxUsedMeso, 4, 8, 1, 1)
        
        rightLayout.addWidget(averageDistroyLabel,2,5,1,1,Qt.AlignRight)
        rightLayout.addWidget(self.averageDistroy,2,6,1,1)
        
        rightLayout.addWidget(maxDistroyLabel,3,5,1,1,Qt.AlignRight)
        rightLayout.addWidget(self.minDistroy,3,6,1,1)
        
        rightLayout.addWidget(minDistroyLabel,4,5,1,1,Qt.AlignRight)
        rightLayout.addWidget(self.maxDistroy,4,6,1,1)
