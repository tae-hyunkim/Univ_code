...
class Sim_GUI(Upgrader):
    ...
    def initUI(self):
        ...
        self.PC_check = QCheckBox("PC방 유무")
        self.sunday_check = QCheckBox("선데이 메이플 유무")
        self.starcatch_check = QCheckBox("스타캐치")
        self.unbreakable_check = QCheckBox("파괴방지")
        emptyPlace = QLabel(" ", self)
                                      # 현재까지의 데이터를 초기화하는 버튼 추가
        hbox01.addStretch(1)
        hbox01.addWidget(level)
        hbox01.addWidget(self.selectedLevel)
        hbox01.addWidget(self.PC_check)
        hbox01.addWidget(self.sunday_check)
        hbox01.addWidget(emptyPlace)
        hbox01.addWidget(self.starcatch_check)
        hbox01.addWidget(self.unbreakable_check)
        hbox01.addStretch(1)
        ...
        self.nowStar = QComboBox()
        for i in range(25):
            self.nowStar.addItem(str(i))
        self.mvp_label = QLabel("MVP: ")
        self.mvp_combo = QComboBox()
        self.mvp_combo.addItem("Normal")
        self.mvp_combo.addItem("Bronze")
        self.mvp_combo.addItem("Silver")
        self.mvp_combo.addItem("Gold")
        self.mvp_combo.addItem("Diamond")
        self.mvp_combo.addItem("Red")
        hbox02.addStretch(1)

        hbox02.addWidget(nowStarLabel)
        hbox02.addWidget(self.nowStar)
        hbox02.addWidget(self.mvp_label)
        hbox02.addWidget(self.mvp_combo)
        hbox02.addWidget(levelButton)
        hbox02.addWidget(reset)
        hbox02.addStretch(1)
        # 파괴방지, 스타캐치, mvp, pc방, 선데이,
        topLayout.addLayout(hbox01)
        topLayout.addLayout(hbox02)
        ...
        # 체크박스 변경 이벤트
        self.PC_check.stateChanged.connect(self.chkChanged)
        self.sunday_check.stateChanged.connect(self.chkChanged)
        self.starcatch_check.stateChanged.connect(self.chkChanged)
        self.unbreakable_check.stateChanged.connect(self.chkChanged)
    ...
    def chkChanged(self):                                          # 체크박스가 변경되었을 경우
        if self.PC_check.isChecked():                              # PC방 할인 적용/미적용
            self.PCRoom = True
        else:
            self.PCRoom = False
        if self.sunday_check.isChecked():                          # 썬데이 메이플 할인 적용/미적용
            self.sunday = True
        else:
            self.sunday = False
        if self.starcatch_check.isChecked():                       # 스타캐치 강화 확률 보너스 적용/미적용
            self.starcatch = True
        else:
            self.starcatch = False
        if self.unbreakable_check.isChecked():                     # 파괴방지기능, 그리고 그에 따른 메소 추가비용 적용/미적
    ...
    def buttonclicked(self):                                        # 버튼이 클릭됐을 때의 기능
        ...
        if sender.text() == "선택":                                 # 무기 레벨제 선택시
            if self.nowStar.currentText() == "":                    # 에러 방지
                return
            if self.selected:                                       # 아직 선택이 안돼있는 상태일 경우 선택하고 초기값 설정
                ...
                self.weaponImage = self.weaponImage.scaled(150, 150)
                self.displayImage.setPixmap(self.weaponImage)
                self.needMeso.setText(norToAbnor(str(calcMeso(self.upgraded, self.selectedLevel.currentText(), self.PCRoom, self.sunday, self.preventDestroy, self.mvp_combo.currentText()))))
                self.usedMeso.setText("0")
                self.upgraded = int(self.nowStar.currentText())
                self.showUpgradeIndex(self.upgraded)
                self.PC_check.setEnabled(False)                     # 체크박스와 콤보박스를 변경 불가능하도록 비활성화
                self.sunday_check.setEnabled(False)
                self.starcatch_check.setEnabled(False)
                self.unbreakable_check.setEnabled(False)
                self.selectedLevel.setDisabled(True)
                self.nowStar.setDisabled(True)
                self.mvp_combo.setDisabled(True)
                self.checkStars()
                self.show()
        elif self.selected == False:                                # 선택이 됐을 경우 강화나 초기화 기능 사용 가능
            if sender.text() == "강화":                             # 강화 기능
                prevStar = self.upgraded
                nextStar = self.doUpgrade(prevStar, self.starcatch, self.preventDestroy)
                ...
                if nextStar == "-1":                                # 파괴되었을 경우
                    ...
                    if prevStar < int(nextStar):
                        self.historyMessage += "\n*강화 성공* : "+str(prevStar)+" --> "+nextStar          #히스토리에 기록
                        self.historyLabel = QLabel(self.historyMessage)
                        self.upgradeHistory.setWidget(self.historyLabel)
                        vbar = self.upgradeHistory.verticalScrollBar()
                        vbar.setValue(vbar.maximum())
                    elif prevStar > int(nextStar):
                        self.historyMessage += "\n강화 실패(하락) : "+str(prevStar)+" --> "+nextStar      #히스토리에 기록
                        self.historyLabel = QLabel(self.historyMessage)
                        self.upgradeHistory.setWidget(self.historyLabel)
                        vbar = self.upgradeHistory.verticalScrollBar()
                        vbar.setValue(vbar.maximum())
                    else:
                        self.historyMessage += "\n강화 실패(유지) : "+str(prevStar)+" --> "+nextStar      #히스토리에 기록
                        self.historyLabel = QLabel(self.historyMessage)
                        self.upgradeHistory.setWidget(self.historyLabel)
                        vbar = self.upgradeHistory.verticalScrollBar()
                        vbar.setValue(vbar.maximum())
                ...
                self.weaponImage = self.weaponImage.scaled(150, 150)
                self.displayImage.setPixmap(self.weaponImage)
                self.PC_check.setEnabled(True)                      # 체크박스와 콤보박스를 변경 가능하도록 활성화
                self.sunday_check.setEnabled(True)
                self.starcatch_check.setEnabled(True)
                self.unbreakable_check.setEnabled(True)
                self.selectedLevel.setDisabled(False)
                self.nowStar.setDisabled(False)
                self.mvp_combo.setDisabled(False)
                self.historyMessage = "[강화 내역]"                 # 히스토리 청소
                self.historyLabel = QLabel(self.historyMessage)
                self.upgradeHistory.setWidget(self.historyLabel)
                self.upgraded = 0
                self.checkStars()
        ...
        if sender.text() == "시뮬레이션 시작":
            if self.selected == True:                               # 에러 방지
                ...
                for i in range(int(self.tryNums.text())):           # 입력된 값만큼 시도함
                    startStar = int(self.nowStar.currentText())
                    ...
                    while currentStar < toStar:                     # 목표 강화 수치에 도달하는 것이 한번임
                        currentMeso += calcMeso(currentStar, self.selectedLevel.currentText(), self.PCRoom, self.sunday, self.preventDestroy, self.mvp_combo.currentText())
                        nextStar = self.doUpgrade(currentStar, self.starcatch, self.preventDestroy)
                        currentTry += 1
                        if nextStar == "-1":
                            currentStar = 12
                            currentDestroy += 1
    ...
    def showUpgradeIndex(self, nowStars):
        # 무기 사진 오른쪽에 표시하는 정보들
        percent, broken = calcPercent(nowStars, self.starcatch, self.preventDestroy)
        self.currentInfo.clear()
        ...
        if nowStars <= 10:
            self.currentInfo.append("강화 실패(유지) 확률 : " + str(round(100-percent-broken, 4)) + "%")
        elif nowStars <= 11:
            self.currentInfo.append("강화 실패(하락) 확률 : " + str(round(100-percent-broken, 4)) + "%")
        else:
            self.currentInfo.append("강화 실패(하락) 확률 : " + str(round(100-percent-broken, 4)) + "%")
