...
class Calculator(QWidget):
    ...
    def buttonClicked(self):
        ...
        if key == '=':
            ...
            self.display.clear()
        # 이부분에 key in constantList 활용해서 작업을 진행 
        # clacFunction에 함수를 정의해서 constantList의 버튼을 찾고 작업하는 형태로 작성하면 된다. 
        elif key in constantList:
            self.display.setText(self.display.text() + calcFunctions.constantListFunc(key,constantList))
        elif key in functionList:
            n = self.display.text()
            self.display.setText(calcFunctions.functionListFunction(key,functionList,n))
        else:
